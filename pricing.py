...
def predict_product_price_with_range_and_profit(production_cost, transportation_cost_percentage, labour_cost, 
                                                raw_material_cost, rent, advertising, packet_size):
    inflation = get_inflation_rate_india()

    # Calculate total cost
    total_cost = (
        production_cost + labour_cost + raw_material_cost + rent +
        advertising + (production_cost * transportation_cost_percentage)
    )
...
# Model training code
file_path = r"C:\Users\91966\ai_pricing_tool\product_data2.csv"  # Update with your file path
data = pd.read_csv(file_path)

# Ensure 'packet_size' is present in the data
if 'packet_size' not in data.columns:
    np.random.seed(42)
    data['packet_size'] = np.random.choice([50, 100, 150, 200, 250], size=len(data))

# Features and target
X = data[['production_cost', 'transportation_cost_percentage', 'labour_cost',
...
# Create a pipeline and perform GridSearchCV for GradientBoostingRegressor
